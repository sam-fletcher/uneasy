...
func TestComputeDifficultyPure(t *testing.T) {
	t.Run("exchange courtiers: difficulty equals target status (6 - rank)", func(t *testing.T) {
		...
		for _, tc := range testCases {
			t.Run(tc.name, func(t *testing.T) {
				diff, err := game.ComputeDifficultyPure(plan.PlanType, ResolutionData{}, tc.targetRank)
	...
	t.Run("make introductions: difficulty equals 2 + peer_count", func(t *testing.T) {
		...
		for _, tc := range testCases {
			t.Run(tc.name, func(t *testing.T) {
				...
				diff, err := game.ComputeDifficultyPure(model.PlanMakeIntroductions, resData, 0)
	...
	t.Run("spread propaganda: difficulty equals preparer rank", func(t *testing.T) {
		...
		for _, tc := range testCases {
			t.Run(tc.name, func(t *testing.T) {
				diff, err := game.ComputeDifficultyPure(model.PlanSpreadPropaganda, ResolutionData{}, tc.preparerRank)
				require.NoError(t, err, "should not error")
	...
	t.Run("propose decree: difficulty equals preparer power rank", func(t *testing.T) {
		...
		for _, tc := range testCases {
			t.Run(tc.name, func(t *testing.T) {
				diff, err := game.ComputeDifficultyPure(model.PlanProposeDecree, ResolutionData{}, tc.preparerRank)
	...
	t.Run("unsupported plan type returns error", func(t *testing.T) {
		diff, err := game.ComputeDifficultyPure(model.PlanMakeDemands, ResolutionData{}, 1)
	...
	t.Run("rank boundary: min safety check on exchange courtiers", func(t *testing.T) {
		...
		diff, err := game.ComputeDifficultyPure(model.PlanExchangeCourtiers, ResolutionData{}, 6)
